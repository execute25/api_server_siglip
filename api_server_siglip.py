...
# ⚡️ GPU SETUP: Determine the device
...
# ⚡️ GPU STEP 1: Load model and move it to the device
model = AutoModel.from_pretrained("google/siglip-base-patch16-224").to(DEVICE)

model.eval()
load_time = time.time() - start
print(f"✅ Model loaded and moved to {DEVICE} in {load_time:.2f}s")
...
def base64_to_image(base64_str):
    ...
    try:
        # Check if the header is present, if not, assume it's pure base64
        if ',' in base64_str:
...
def preprocess_batch_images(items):
    """Image preprocessing for the batch - kept for design consistency/timing, but actual decoding happens inside threads."""
    images = []
    for item in items:
        try:
...
def process_single_classification(keywords, text, base64_image):
    ...
    ⚡️ GPU STEP 2: Move inputs to device
    """
    try:
        ...
        # ⚡️ GPU STEP 2: Pass inputs and then move the resulting tensors to the DEVICE
        ...
        with torch.no_grad():
            ...
            if hasattr(outputs, 'logits_per_image'):
                ...
                # Calculate logits manually, ensuring embeds are on the correct device
        ...
        # Extract results based on keywords/prompts and probs, move to CPU for float conversion
        probs_cpu = probs.cpu()
        results = [[kw, float(probs_cpu[0][i])] for i, kw in enumerate(keywords)]
    ...
    try:
        ...
        for category_type, config in categories_config.items():
            ...
            with torch.no_grad():
                ...
                # Move to CPU for top_idx and float conversion
                probs_cpu = probs.cpu()
                top_idx = probs_cpu[0].argmax().item()

                if probs_cpu[0][top_idx] > 0.1:
                    style_result[category_type] = {
                        'name': keywords[top_idx],
                        'score': float(probs_cpu[0][top_idx])
                    }
...
async def process_batch_parallel(items, process_function):
    ...
    try:
        ...
        # Note: When using a GPU, the GIL is released during model inference,
        # allowing for thread-based parallelism to still be effective for I/O and
        # pre/post-processing, though model-bound operations will serialize on a single GPU.
        # This thread pool setup is correct for FastAPI's async handling of blocking code.
...
async def root():
    return {
        "status": "ok",
        "version": "10.0-tech-opt1-gpu",
        "device": str(DEVICE), # ⚡️ Added device info
        "batch_support": True,
        "parallel_workers": MAX_WORKERS,
        "tech_support": True,
        "optimization": "No image duplication (1.3x faster) + GPU support",
...
async def classify(request: ClassificationRequest):
    ...
    try:
        ...
        if request.base64_image:
            ...
            # Add a data URL prefix if it's not present (common for external images)
        ...
        result["method"] = "siglip_v10_tech_opt1_gpu"
...
async def classify_batch(request: BatchRequest):
    ...
    try:
        ...
        response = {
            ...
            "method": "parallel_batch_v10_tech_opt1_gpu"
        }
...
async def classify_style_batch(request: StyleBatchRequest):
    ...
    try:
        ...
        response = {
            ...
            "failed_items": error_count,
            "avg_time_per_item": total_time / len(request.items),
            "method": "parallel_style_batch_v10_tech_opt1_gpu"
        }
...
if __name__ == "__main__":
    logger.info(f"🚀 Starting API with {MAX_WORKERS} parallel workers on device: {DEVICE}")
